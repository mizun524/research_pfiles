# calculate the beampattern of Spherical Harmonics Beamforming
...
for ifreq in freq_analyse:
    ...
    for itime in range(len(time_f)):
        # SH coeffients for the P
        pnm1 = np.dot(s1[idx, itime, :], np.linalg.pinv(Y_mic))
        pnm2 = np.dot(s2[idx, itime, :], np.linalg.pinv(Y_mic))
        pnm3 = np.dot(s3[idx, itime, :], np.linalg.pinv(Y_mic))
        # SH coefficients for the beampattern
        bnm1 = pnm1 / b
        bnm2 = pnm2 / b
        bnm3 = pnm3 / b
        # beampattern
        beamp1[:, itime] = np.abs(np.dot(bnm1, Y_obs))
        beamp2[:, itime] = np.abs(np.dot(bnm2, Y_obs))
        beamp3[:, itime] = np.abs(np.dot(bnm3, Y_obs))
